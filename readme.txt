...
The App Maker adds an App custom post type that allows you to build a cross-device mobile web app out of your content.
...
The App Maker uses [Sencha Touch](http://www.sencha.com/products/touch/) as the Javascript framework for building the app.  The App Maker adds an App custom post type allowing you to build as many apps as you want.  The apps themselves are rendered using shortcodes.  For a Sample App, please see the Frequently Asked Questions section.  

== Installation ==

1. The App Maker is available from the WordPress repository at [wordpress.org/extend/plugins/the-app-maker](http://wordpress.org/extend/plugins/the-app-maker)
...
= How does it work? = 
An app build with The App Maker is built as a custom post type using shortcodes.  When the custom app renders, it completely bypasses the WordPress templating and builds its own HTML page, from the <!DOCTYPE> to the </html>.  As it parses the custom app, it builds up settings within an object that is globally available to any other plugin via a PHP statement like `$the_app = & TheAppMaker::getInstance();`.  The available shortcodes for building an app are listed below.  See the sample app later in this section for more details: 
...
function borealis_catalogue_app($atts){
	...
	add_action('the_app_maker_print_manifest','borealis_app_print_manifest');
}
...
You may well want to do things with your app(s) that aren't possible with The App Maker natively.  Fortunately, there are many hooks available to allow you to do custom things.  

Commonly used filters include:

* TheAppMaker_models - change the models for the app
* TheAppMaker_stores - change the stores for the app
* TheAppMaker_scripts - add additional Javascript to the app

Commonly used actions include
* the_app_maker_print_stylesheets - print additional stylesheets
* the_app_maker_print_scripts - print additional Javascript scripts
* the_app_maker_print_manifest - add files to the Cache Manifest
